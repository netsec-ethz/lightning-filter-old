...
func sendHello(daemonAddr, dispatcherSocket string, localAddr snet.UDPAddr, remoteAddr snet.UDPAddr) {
	var err error
	ctx := context.Background()

	sdc, err := daemon.NewService(daemonAddr).Connect(ctx)
	if err != nil {
...
func main() {
	...
	flag.StringVar(&daemonAddr, "daemon", "", "SCIOND address")
	...
	sendHello(daemonAddr, dispatcherSocket, localAddr, remoteAddr)
}
